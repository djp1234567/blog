<template>
  <div class="blogList my-8 flex flex-col items-center justify-center">
    <a class="blog flex flex-col md:flex-row md:items-center md:justify-between" 
                v-for="item in posts" 
    ...
    <div class="pagination select-none">
        <button class="absolute left-0"  v-if="pageCurrent > 1"
            @click="go(pageCurrent - 1)"   >Previous page</button>
        <div v-if="pagesNum > 1">{{ `${pageCurrent}/${pagesNum}` }}</div>
        <button class="absolute right-0" v-if="pageCurrent < pagesNum" 
            @click="go(pageCurrent + 1)">Previous page</button>
    </div>
</template>
